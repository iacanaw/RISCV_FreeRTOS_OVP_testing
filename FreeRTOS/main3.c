...
int main( void )
{
    ...
    *************************************************************************/
    ...
    void *fibo = pvPortMalloc(1073741824);

    /* Create the two test tasks. */
 	 xTaskCreate( vUartTestTask1, "UArt1", 1000, NULL, uartPRIMARY_PRIORITY, NULL );

    //           task  taskname stack  parameter  priority      handler
    xTaskCreate( myTask1, "task1", 1000, NULL, tskIDLE_PRIORITY, NULL );
...
static void vUartTestTask1( void *pvParameters )
{
	...
	for( ;; )
	{
		UART_polled_tx_string( &g_uart, (const uint8_t *)"Task - 1 - oie\r\n" );
	    vTaskDelay(2000);
	}
}
...
static void vUartTestTask2( void *pvParameters )
{
	...
	for( ;; )
	{
		UART_polled_tx_string( &g_uart, (const uint8_t *)"Task - testete\r\n" );
	    vTaskDelay(2000);
	}
}
...
static void myTask1( void *pvParameters )
{
    ...
    for ( ; ;){
        char buffer[100];
        snprintf(buffer, sizeof(buffer), "Fibonnaci de %d é igual a %d\r\n", i, fibonnaci(i));
        UART_polled_tx_string(&g_uart, (const uint8_t *) buffer);
        vTaskDelay(5);
        i++;
    }
}
